			...
			for(int i=DV.length-1;i>=0;i--)
				if(DV[i].abbr.length()>0)
				{
					...
					if(denom<absoluteAmount)
					{
						...
						diff=Math.abs(absoluteAmount-denom*num);
...
//			return ""+lowAmt;
	...
	public Coins makeBestCurrency(String currency, double absoluteValue, ItemCollection container)
	{
	...
	public void addMoney(ItemCollection container, String currency, double absoluteValue)
	{
		Vector<Coins> V=makeAllCurrency(currency,absoluteValue);
		for(int i=0;i<V.size();i++)
		{
			Coins C=V.elementAt(i);
			container.addItem(C);
			C.putCoinsBack();
		}
	}
	...
	public void giveSomeoneMoney(MOB banker, MOB customer, String currency, double absoluteValue)
	{
		...
		for(int i=0;i<V.size();i++)
		{
			...
			banker.getItemCollection().addItem(C);
			...
			if(last.startsWith("COINS;"))
			{
				if((bankName==null)||(bankName.length()==0)||(bankName.equals(D.section)))
				{
					...
					if((absoluteAmount>0.0)||(value>=(-absoluteAmount)))
					{
						...
						if(C!=null)
							CMLib.database().DBReCreateData(owner,D.section,D.key,"COINS;"+CMLib.coffeeMaker().getPropertiesStr(C));
						else
