...
public class Sense extends StdLibrary
{
...
/*
	...
	{ return (E!=null)&&(E.body().getMyRace().racialCategory().equalsIgnoreCase("Vegetation")); }
	public boolean isSwimmingInWater(Environmental E)
	{
		if(!isSwimming(E)) return false;
		Room R=CMLib.map().roomLocation(E);
		if(R==null) return false;
		return ((R.domain().contains(Room.Domain.Underwater))||(R.domain().contains(Room.Domain.WaterSurface)));
	}
	...
	public boolean aliveAwakeMobile(MOB mob, boolean quiet)
	{
		...
		if(mob.amDead()||(mob.charStats()==null)||(mob.charStats().getPoints(CharStats.STAT_HITPOINTS)<0))
		{
			if(!quiet)
	...
	{
		return (!isSitting(mob))&&(!isSleeping(mob))&&(!isLayingDown(mob));
	}
	...
	public boolean isBoundOrHeld(Environmental E)
	{
		...
		if((E.envStats().disposition()&EnvStats.IS_BOUND)==EnvStats.IS_BOUND)
			return true;
		return flaggedAnyAffects(E,Effect.FLAG_BINDING|Effect.FLAG_PARALYZING).size()>0;
	}
	...
	public boolean canBeSeenBy(Environmental seen , MOB seer)
	{
		...
		if((seer!=null)
		&&(CMath.bset(seer.playerStats().getBitmap(),PlayerStats.ATT_SYSOPMSGS)))
			return true;
	...
	public boolean canFreelyBehaveNormal(Tickable affecting)
	{
		if(affecting instanceof MOB)
		{
			...
			||(monster.charStats().getPoints(CharStats.STAT_HITPOINTS)<((int)Math.round(monster.charStats().getMaxPoints(CharStats.STAT_HITPOINTS)/2.0))))
				return false;
			return true;
		}
		return false;
	...
	public Vector domainAnyAffects(Environmental E, int domain)
	{
		Vector V=new Vector();
		if(E!=null)
			if(domain>Effect.ALL_ACODES)
			{
				for(int a=0;a<E.numEffects();a++)
				{
					Effect A=E.fetchEffect(a);
					if((A!=null)&&((A.classificationCode()&Effect.ALL_DOMAINS)==domain))
					{ V.addElement(A);}
				}
			}
			else
			...
			{
				Effect A=E.fetchEffect(a);
				if((A!=null)&&((A.classificationCode()&Effect.ALL_ACODES)==domain))
				{ V.addElement(A);}
			}
		return V;
	}
	...
	public boolean enchanted(Item I)
	{
		...
		{
			Effect A=I.fetchEffect(i);
			if((A!=null)
			&&((A.classificationCode()&Effect.ALL_ACODES)!=Effect.ACODE_PROPERTY)
			&&((A.classificationCode()&Effect.ALL_ACODES)!=Effect.ACODE_DISEASE)
			&&((A.classificationCode()&Effect.ALL_ACODES)!=Effect.ACODE_POISON))
				return true;
		}
		return false;
	}
	...
	{
		if(A==null) return "";
		return Effect.ACODE_DESCS[A.classificationCode()&Effect.ALL_ACODES];
	}
	public String getEffectDomain(Effect A)
	{
		if(A==null) return "";
		return Effect.DOMAIN_DESCS[(A.classificationCode()&Effect.ALL_DOMAINS)>>5];
	}
	public int getEffectType(String name)
	{
		for(int i=0;i<Effect.ACODE_DESCS.length;i++)
			if(name.equalsIgnoreCase(Effect.ACODE_DESCS[i]))
				return i;
		return -1;
	}
	public int getEffectDomain(String name)
	{
		for(int i=0;i<Effect.DOMAIN_DESCS.length;i++)
			if(name.equalsIgnoreCase(Effect.DOMAIN_DESCS[i]))
				return i<<5;
		return -1;
	}
	...
	public boolean isATrackingMonster(Environmental E)
	{
		...
		if((E instanceof MOB)&&(((MOB)E).isMonster()))
			return flaggedAffects(E,Effect.FLAG_TRACKING).size()>0;
		return false;
	}
	...
	{ return (E!=null)&&((E.envStats().disposition()&EnvStats.IS_LAYINGDOWN)==EnvStats.IS_LAYINGDOWN); }
