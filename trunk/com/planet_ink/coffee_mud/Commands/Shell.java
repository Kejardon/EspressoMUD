...
public class Shell extends StdCommand
{
		...
		for(int v=V.size()-1;v>=0;v--)
		{
			...
			if((CF.isDirectory())&&(CF.exists()))
			{
				...
				while(x<=dirs.size())
				{
					...
					if(((CMFile)dirs.elementAt(x)).getLocalPathAndName().length()<CF.getLocalPathAndName().length())
						x++;
					else
					{
		...
		throws java.io.IOException
	{
		...
		case 1: // copy
		{
			...
			if((dirs.length==1)&&(!target.trim().startsWith("//")))
				target="//"+target.trim();
			...
			for(int d=0;d<ddirs.size();d++)
			{
				...
				CMFile DF=DD;
				target=DD.getLocalPathAndName();
				if(DD.isDirectory())
				{
					String name=SF.getName();
					if((opts.recurse)&&(opts.preservePaths))
					{
						String srcPath=SF.getLocalPathAndName();
		...
		case 2: // cd
		{
			...
			String changeTo=newDir.getLocalPathAndName();
			if(!newDir.exists())
			{
		...
		case 4: // type
		{
			...
			for(int d=0;d<dirs.length;d++)
			{
				...
				if(mob.session()!=null)
				{
					mob.session().colorOnlyPrintln("\n\r^xFile /"+CF.getLocalPathAndName()+"^.^N\n\r");
		...
		case 6: // findfiles
		{
			...
			for(int d=0;d<dirs.length;d++)
			{
				...
				{
					msg.append(" ");
					msg.append("^w"+entry.getLocalPathAndName());
					msg.append("\n\r");
				}
			}
			if(mob.session()!=null)
		...
		case 8: // edit
		{
			...
			while((mob.session()!=null)&&(!mob.session().killFlag()))
			{
				...
				else
				{
					...
					case 'L':
					{
						StringBuffer list=new StringBuffer("File: "+file.getLocalPathAndName()+"\n\r");
	...
	public String desc(CMFile CF){ return "Local file '/"+CF.getLocalPathAndName()+"'";}
